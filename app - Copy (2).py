...
def send_message():
    ...
    messages_data[user].append(message)
...
@app.route('/logout')
def logout():
    # No server-side session is needed since everything is handled on the client-side (localStorage)
    # Simply return the same page to simulate logout (redirect to name entry prompt)
